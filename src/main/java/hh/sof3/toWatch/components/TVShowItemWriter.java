...
@Component
public class TVShowItemWriter implements ItemWriter<Media> {
    private final JdbcTemplate jdbcTemplate;

    public TVShowItemWriter(DataSource dataSource) {
    ...
    public void write(@NonNull Chunk<? extends Media> items) throws Exception {
        for (Media item : items) {
            jdbcTemplate.update(
                    "INSERT INTO tvshow (show_id, show_type, title, director, starring, country, date_added, release_year, rating, duration, listed_in, description) "
                            +
                            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
